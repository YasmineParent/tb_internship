...
> **[Figure 1 — PLACEHOLDER]** Two panels on heart. (a) Side-by-side integer scorecards: vanilla top-1 card vs causal top-1 card, swapped features highlighted — vanilla uses slope=3 (q=0.00) and age bins (q=0.05) as top features; causal replaces them with exercise-induced angina (q=0.75) and maximum heart rate (q=0.50). (b) Rashomon pool cloud: test AUC vs support q-mass per pool member, vanilla scattered at low q-mass, causal concentrated at high q-mass at equal AUC. Source: `experiments/causal_prior/real/fig_scorecard.py` → `results/causal_prior/real/fig_scorecard.png`.
