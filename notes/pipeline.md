...
| deploy in seconds; $\mu$-tuning is a one-time offline cost of minutes (§4.4) | measured | `runtime_bench.py` | 5 benchmarks |
...
We report at a sparsity budget of $K=2k^\star$, beyond the true support so the extra slots are where vanilla admits confounded correlates. The choice is not load-bearing: the K-budget ablation (Figure 3) shows the prior's gain over vanilla is roughly flat across $K$ from $k^\star$ to $3k^\star$, present already at $K=k^\star$. Absolute recovery declines with $K$ (more slots admit more non-causal features) but the gain does not, so the method's benefit does not depend on tuning the budget.
...
| GES (global) | 0.66 | 0.90 | 0.73 |
| PC (global) | 0.62 | 0.21 | 0.42 |
| bootstrap-$L_1$ (predictive) | 0.30 | 0.75 | 0.41 |

Causal discovery recovers the true support. IAMB, which targets the Markov blanket directly, does so near-perfectly (F1 0.99). GES recovers the bulk of it (F1 0.73) but includes some non-parent ancestors at lower edge densities. PC degrades sharply as the graph becomes denser: constraint-based discovery is sample-starved on dense Gaussian DAGs and F1 falls to 0.42 at the Table 4.1 operating point. The predictive source has low precision because $L_1$ regularization includes confounders alongside causes and cannot distinguish them. The scorecard recovery sweep (`recovery_sweep.py`) quantifies the soft prior's effect on S_precision (fraction of selected features in $S^\star$) as a function of $\mu$. Support recovery is not a predictive quantity, so a CV-picked $\hat\mu$ (which targets held-out loss or AUC) is uninformative for it on balanced synthetic data; instead we report recovery over a fixed $\mu_\text{rel}$ grid and quote the operating value $\mu_\text{rel}=1$, above the do-no-harm threshold. Figure 3 shows recovery is flat across a broad $\mu$ band, so this choice is not load-bearing: a fixed $\mu$ matches the per-cell best-case $\mu$ (post-hoc argmax over the grid) within the resolution of the sweep. Both causal sources outperform the vanilla floor at every confounding density. The deployed MB source, IAMB, gains +0.15 to +0.44 over vanilla across $p_\text{edge}$, widening with confounding; GES tracks it (+0.07 to +0.29). The predictive bootstrap-$L_1$ sits at or below the vanilla floor (down to $-0.08$ at high confounding), never cleanly separating causes from the confounded correlates: the predictive source actively costs recovery where a causal one gains it, which is the §6.1 mediation point in its sharpest form. The gain is robust to the sparsity budget: across $K\in\{k^\star,1.5k^\star,2k^\star,3k^\star\}$ the IAMB gain over vanilla holds at roughly +0.13 to +0.17, present already at $K=k^\star$ (the K-budget ablation, Figure 3); the budget changes absolute recovery, not the gain. The benefit is largest at small-to-moderate $n$ (IAMB +0.23 at $n=150$) and shrinks as $n$ grows, though IAMB keeps a margin (+0.10) even at $n=1000$ where the GES advantage has mostly closed. The p-sweep is sharpest: at $p=50$ the IAMB advantage grows to +0.48 (GES +0.35) because more candidate features give the prior more room to concentrate the $K$ slots on $S^\star$.

**q-source robustness** (`q_robustness.py`, anchor cell $p=30, n=300, k^\star=5, p_\text{edge}=0.2$, fixed $\mu_\text{rel}=1$, 20 seeds). Sources are compared across five paradigms: constraint-global (PC_stable), constraint-local (IAMB, GS, inter-IAMB), score-based (GES), continuous-optimization (DAGMA), predictive-stability (bootstrap-$L_1$), and two controls (uniform, adversarial). Per-source AUC is flat (0.992 to 0.997 across non-adversarial sources, spread 0.007); the prior causes no accuracy degradation regardless of which $q$ is supplied, consistent with the do-no-harm property of §3. Markov-blanket recovery (F1 against $S^\star$) tracks $q$ quality. The MB-local learners rank highest because they target exactly the Markov blanket the prior encodes: GS achieves F1 0.80, IAMB 0.77, inter-IAMB 0.76. GES (score-based, global, 0.65) follows. Uniform $q$ (0.58) is the vanilla floor, with DAGMA (0.58) and PC_stable (0.54) at or just below it. Bootstrap-$L_1$ (0.51) sits below the floor, because the predictive source at moderate confounding does not concentrate evidence selectively on $S^\star$. The adversarial source (0.06) collapses and carries the lowest AUC of any source (0.975 against a non-adversarial mean of 0.995); it is the only source that both degrades recovery and nudges accuracy down.

**q-corruption graceful degradation** (`q_corruption.py`, same anchor, starting from the GES prior, fixed $\mu_\text{rel}=1$, 20 seeds, 11 levels). The experiment interpolates $q$ from the GES prior toward uniform noise and scores both S_precision and held-out AUC at each level. S_precision starts at 0.55 (corruption 0, the GES source) and declines gradually to 0.32 at corruption 1.0 (pure noise $q$, the no-information floor). AUC stays flat throughout: 0.993 at corruption 0, 0.988 at corruption 1.0, a total spread of 0.003. Accuracy is protected without any adaptive tuning: at a fixed $\mu$ the loss still dominates the ranking, so even a fully corrupted $q$ costs accuracy nothing, while recovery slides to the floor smoothly rather than catastrophically.

> **[Figure 3 — PLACEHOLDER]** Two parts, no CV (recovery is not a predictive quantity, so a CV-picked $\mu$ is arbitrary on synthetic data; we characterise recovery as a function of $\mu$ directly). **(a) Mechanism + robustness:** left, S_precision vs $\mu_\text{rel}$ at the anchor cell for every source — IAMB and GES rise and plateau, bootstrap-$L_1$ declines *below* the vanilla floor (selectivity is the mediator), uniform is flat (do-no-harm), adversarial collapses; right, S_precision at a fixed $\mu_\text{rel}=1$ vs the per-regime best-case $\mu$ across $p_\text{edge}$ — they nearly coincide, so precise $\mu$ tuning buys little. **(b) Across-regime:** S_precision vs {n, p, p_edge, k_star} at fixed $\mu_\text{rel}=1$, IAMB/GES/bootstrap-$L_1$/vanilla floor. **(c) K-budget:** S_precision vs $K/k^\star\in\{1,1.5,2,3\}$ — IAMB and GES hold a roughly constant margin above the vanilla floor at every budget (the gain is flat across $K\geq k^\star$), while absolute recovery falls with $K$. Source: `src/causal_prior/figures/recovery.py` → `figures/main/fig3{a_mechanism,b_robustness,c_kbudget,d_regime}.pdf` + `fig3_legend.pdf`.
...
> **[Table 1 — PLACEHOLDER]** Five public benchmarks: test AUC (mean ± std) and Nogueira selection stability, one row per benchmark, columns: vanilla / ours (GES) / ours (IAMB) / ours (IAMB, Fisher-Z) / CFS valid (mi-cg) / CFS naive (Fisher-Z) / CFS naive (HITON). Source: `src/causal_prior/cfs_results.py` → `results/causal_prior/cfs/`.
...
`experiments/causal_prior/real/runtime_bench.py` decomposes the cost of building one deployed scorecard into discovery (the step shared with hard CFS), one FasterRisk fit, and the AUC-CV grid that selects $\mu$ (5-fold $\times$ 16-$\mu$).

| dataset | $n$ | deploy: discovery + fit (s) | + AUC-CV $\mu$-tuning (s) | total build (s) |
|---|---|---|---|---|
| fico | 10459 | 21 | 540 | 561 |
| heart | 270 | 2.1 | 66 | 68 |
| mammographic | 831 | 1.1 | 42 | 43 |
| ilpd | 583 | 2.1 | 84 | 86 |
| german | 1000 | 2.2 | 78 | 80 |

Two costs, with different status. Producing a scorecard given a chosen $\mu$ is seconds even at $n=10^4$, and speed-matched to hard CFS because discovery is the shared bottleneck; serving a fitted card is an integer dot product, free for every method. Selecting $\mu$ by AUC-CV is the genuine added cost: minutes, dominated by the grid, and it is paid once per dataset offline (not per prediction). We report it plainly rather than claim a speed advantage the method does not have over its baselines: ours is slower to *build* than vanilla or hard CFS by exactly this tuning step. The cost is small in absolute terms for a deploy-once artifact, and the tuned $\mu$ delivers the do-no-harm AUC of Section 4.2; it is not free.
...
> **[Figure 1 — PLACEHOLDER]** Two panels. (a) Side-by-side integer scorecards on 30-day diabetes readmission (130 US hospitals, n=100k): vanilla uses insurance codes (`payer_code=DM` at −3 pts, q=0.00; `payer_code=HM` at −1 pt, q=0.00) and age bracket (q=0.00) as risk factors; causal replaces them with discharge disposition (where patient is sent after discharge, q=0.09) and metformin status (q=0.06). Causal AUC 0.642 vs vanilla 0.626. (b) Rashomon pool cloud on heart: test AUC vs support q-mass per pool member, vanilla scattered at low q-mass (~0.19), causal concentrated at high q-mass (~0.55) at equal AUC. Source: `src/causal_prior/figures/scorecard.py` (refit: `experiments/causal_prior/real/scorecard_fit.py`) → `figures/main/fig1a_scorecard.tex` (latex FasterRisk-style scorecards, points + SCORE→RISK) + `fig1b_rashomon.pdf`.
