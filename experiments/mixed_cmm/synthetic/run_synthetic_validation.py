...
Usage:
    ...
    python experiments/mixed_cmm/synthetic/run_synthetic_validation.py --n_obs 8 --n_seeds 20 --n_subsample_runs 30
    python experiments/mixed_cmm/synthetic/run_synthetic_validation.py --smoke       # quick stability-selection sanity check
    python experiments/mixed_cmm/synthetic/run_synthetic_validation.py --single_run  # no subsampling, checks if edges findable at all
"""
import sys
import os
import argparse
...
from src_tb.causal_recovery.cmm_utils import run_cmm, subsample_cmm, get_stable_edges, edge_stability

REPO_ROOT = Path(__file__).resolve().parents[3]


def run_experiment(n_obs: int, n_seeds: int, n_subsample_runs: int, threshold: float, use_logistic: bool, output_path: str = None) -> list[dict]:
    ...
    for seed in range(n_seeds):
        ...
        cmm_list = subsample_cmm(data.X, data.forbidden_edges, n_runs=n_subsample_runs,
...
def run_single(n_obs: int, n_seeds: int, use_logistic: bool) -> list[dict]:
    """Single run per seed. No subsampling, no stability threshold. Checks if edges are findable at all."""
    label = 'logistic' if use_logistic else 'gaussian'
    records = []
    for seed in range(n_seeds):
        print(f"  [{label}] seed {seed + 1}/{n_seeds}", flush=True)
...
def print_summary(records_old: list[dict], records_new: list[dict]):
    ...
    if all(c in df_old.columns for c in freq_cols):
        print("\nMean subsample selection frequency of true edges")
...
def parse_args():
    ...
    parser.add_argument('--n_subsample_runs', type=int, default=100)
    parser.add_argument('--threshold',   type=float, default=0.5)
    parser.add_argument('--smoke',       action='store_true', help='quick stability-selection sanity check (overrides defaults)')
    parser.add_argument('--single_run',  action='store_true', help='no subsampling, single CMM fit per seed')
...
def main():
    ...
    if args.smoke:
        args.n_obs, args.n_seeds, args.n_subsample_runs = 4, 2, 3
        print("Smoke test (n_obs=4, n_seeds=2, n_subsample_runs=3)", flush=True)

    if args.single_run:
        print(f"Single run (n_obs={args.n_obs}, n_seeds={args.n_seeds}, no subsampling)", flush=True)
    ...
    with open(output_dir / 'config.txt', 'w') as f:
        f.write(f"n_obs={args.n_obs}\nn_seeds={args.n_seeds}\nn_subsample_runs={args.n_subsample_runs}\nthreshold={args.threshold}\n")

    print("Gaussian noise (old model)", flush=True)
    records_old = run_experiment(args.n_obs, args.n_seeds, args.n_subsample_runs, args.threshold, use_logistic=False,
                                output_path=str(output_dir / 'results_gaussian.csv'))

    print("Logistic FLXMRglm (new model)", flush=True)
    records_new = run_experiment(args.n_obs, args.n_seeds, args.n_subsample_runs, args.threshold, use_logistic=True,
